...
function findChosenTerm(elem, chosenMark){
    /* Ajoute comme attribut de chosenMark le code du terme choisi */
...
function colorRelatives(elem, chosenMark){
    //Colore le terme elem si il est fils ou père du terme représenté par le mark choisi
    if (chosenMark != undefined && chosenMark.term != undefined){
	//chosenMark.term est undefined si on a cliqué sur un cluster ou data
...
function createLabel(elem, m, y0, y1, w, h){
    ...
    //création du label pour le rectangle selectionné
    if ((m != undefined ) && ( elem.markt.id() == m.id())){
	    ...
	    else { //autrement séparation en plusieurs labels
		var tmp = name.split(" ");
    ...
    //création du label pour les autres rectangles
